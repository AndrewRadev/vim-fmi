...
describe Activation do
  ...
  it "validates the password's confirmation" do
    activation = Activation.new create(:sign_up, :assigned)
  ...
  describe "on submission" do
    let(:sign_up) { create(:sign_up, :assigned) }
    let(:activation) { Activation.new(sign_up) }
    let(:valid_attributes) { { password: 'password', password_confirmation: 'password' } }
